...
> *"면적 터치 위아래 드래그 조작, 점수 영역 탭 시 소분류 확대 진입, 4점 이하 적응형 확대 시 배경 눈금 라벨 동적 재스케일링, 소분류 쐐기 실시간 미리보기, 하단 미니 다이얼 림, 그리고 옥타곤 배경 360도 통합 레이더 차트 완성을 제공합니다."*
...
* **Dynamic Re-calibrated Background Scale (배경 눈금 동적 재스케일링)**: 대분류 점수가 4점 이하인 경우 시각적 확대와 함께 뒷배경의 원형 눈금 숫자도 실제 대분류 점수(예: 1.0점, 2.0점, 3.0점 상한)에 맞춰 정밀하게 재스케일링.
...
* **배경 눈금 라벨 동적 전환 (Dynamic Scale Labeling)**:
  * 확대 렌즈 모드 진입 시 뒷배경의 $2, 4, 6, 8, 10$ 기본 눈금이 **대분류 점수 기준의 정밀 눈금(예: $1.0\text{점}, 2.0\text{점}, 3.0\text{점 (상한)}$)**으로 자연스럽게 모핑되어 상한선과 눈금이 100% 일치합니다.
...
| :--- | :--- | :--- | :---: | :--- |
| **FR-01** | **인차트 조작** | **배경 눈금 동적 재스케일링**| **P1** | 4점 이하 확대 시 배경 눈금 숫자가 대분류 점수 기준(예: 1점, 2점, 3점 상한)으로 전환 |
...
| :--- | :--- | :--- | :---: |
| **1단계** | **설계 & 프로토타입** | • PRD 및 배경 눈금 동적 재스케일링 규칙 확립<br>• 옥타곤 롱프레스 통합 HTML 프로토타입 | 🟢 **완료** |
...
* **배경 눈금 동적 재스케일링**: 대분류가 3점일 때 줌인하면 배경 눈금도 1.0점, 2.0점, 3.0점(상한)으로 정밀 전환
