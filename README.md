...
> *"면적 터치 위아래 드래그 조작, 점수 영역 탭 시 소분류 확대 진입, 소분류 줌인 상태 하단 중앙 미니 반원 다이얼 림 네비게이션, 그리고 옥타곤 배경 360도 통합 레이더 차트 완성을 제공합니다."*
...
2. [핵심 아키텍처 규칙 5대 원칙](#2-핵심-아키텍처-규칙-5대-원칙)
   ...
   - 2.3 [소분류 줌인 상태 하단 중앙 미니 반원 다이얼 림(Mini Dial Rim)](#23-소분류-줌인-상태-하단-중앙-미니-반원-다이얼-림mini-dial-rim)
...
### 2.3 소분류 줌인 상태 하단 중앙 미니 반원 다이얼 림(Mini Dial Rim)

* 소분류 줌인 상태(`zoomProgress > 0.2`) 시, 화면 하단 중심$(cx, cy)$에 **동심원 비율의 작은 반원 다이얼 림($R_{\text{dial}} \approx R_{\min} \times 0.92$)**이 렌더링됩니다.
* 사용자가 상단에서는 소분류 쐐기 레일의 점수 조절에 집중하고, **하단 미니 반원 다이얼 영역을 좌우로 슥슥 돌리면 1단계 휠 뷰로 줌아웃되며 다음 대분류로 편안하게 회전 전환**됩니다.
...
| **FR-02** | **휠 네비게이션**| **하단 미니 반원 다이얼**| **P1** | 소분류 줌인 상태에서 하단 중앙 미니 반원을 돌려 대분류 손쉬운 전환 |
...
| :--- | :--- | :--- | :---: |
| **1단계** | **설계 & 프로토타입** | • PRD 및 미니 반원 다이얼 림 규칙 확립<br>• 옥타곤 롱프레스 통합 HTML 프로토타입 | 🟢 **완료** |
