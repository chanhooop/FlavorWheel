...
> *"면적 터치 위아래 드래그 조작, 점수 영역 탭 시 소분류 확대 진입, 대분류 영역 내 소분류 10점 만점 매핑, 소분류 쐐기 실시간 미리보기, 하단 미니 다이얼 림, 그리고 옥타곤 배경 360도 통합 레이더 차트 완성을 제공합니다."*
...
2. [핵심 아키텍처 규칙 5대 원칙](#2-핵심-아키텍처-규칙-5대-원칙)
   ...
   - 2.3 [대분류 점수 영역 바운딩 & 소분류 10점 만점 매핑 모델](#23-대분류-점수-영역-바운딩--소분류-10점-만점-매핑-모델)
...
* **Bounded Area with 10-Point Scale (대분류 영역 내 10점 만점 매핑)**: 소분류의 선택/조작 영역 상한은 대분류 점수 영역($R_{\text{intensity}}$)까지로 제한되지만, 그 상한선 도달 시 점수는 모두 10점 만점으로 계산 및 표기.
...
### 2.3 대분류 점수 영역 바운딩 & 소분류 10점 만점 매핑 모델

* **기하학적 바운딩**: 소분류 쐐기 레일의 터치 및 렌더링 상한은 **해당 대분류가 설정한 점수 영역($R_{\text{intensity}}$)**까지로 제한됩니다.
* **10점 만점 매핑 수식**: 대분류의 점수 크기(예: 7.0점)와 무관하게, 대분류 영역의 바닥($R_{\min}$)이 0.0점, 상한선($R_{\text{intensity}}$)에 도달했을 때를 **10.0점 만점**으로 계산합니다:
  $$\text{score}_{\text{sub}} = \frac{\text{dist} - R_{\min}}{R_{\text{intensity}} - R_{\min}} \times 10.0 \quad (\in [0.0, 10.0])$$
  $$\text{fillR}_{\text{sub}} = R_{\min} + (R_{\text{intensity}} - R_{\min}) \times \frac{\text{score}_{\text{sub}}}{10.0}$$
...
graph TB
    subgraph Client ["Flutter Client (Offline-First Interactive Architecture)"]
        BoundedScaleEngine["대분류 영역 바운딩 & 10점 매핑 레일 엔진"]
...
| :--- | :--- | :--- | :---: | :--- |
| **FR-01** | **인차트 조작** | **영역 바운딩 & 10점 매핑**| **P1** | 소분류 쐐기 상한은 대분류 영역까지 제한되나 점수는 10점 만점으로 매핑 |
...
| :--- | :--- | :--- | :---: |
| **1단계** | **설계 & 프로토타입** | • PRD 및 영역 바운딩 10점 만점 매핑 규칙 확립<br>• 옥타곤 롱프레스 통합 HTML 프로토타입 | 🟢 **완료** |
...
* **대분류 영역 바운딩 & 10점 만점 매핑**: 소분류 레일은 대분류 영역 안에서만 확장되지만 점수는 10점 만점으로 평가
