...
> *"면적 터치 위아래 드래그 조작, 점수 영역 탭 시 소분류 확대 진입, 4점 이하 적응형 확대 시 배경 눈금 동적 재스케일링, 하단 미니 다이얼 림, 그리고 완성된 차트에서 대분류 레이더만 우선 노출 ➔ 탭 시 소분류 파이 전환 ➔ 개별 파이 탭 시 대폭 확대(Mega Pop-out) 인터랙션을 제공합니다."*
...
2. [핵심 아키텍처 규칙 5대 원칙](#2-핵심-아키텍처-규칙-5대-원칙)
   ...
   - 2.6 [완성형 차트 계층 뷰: 대분류 레이더 ➔ 소분류 파이 ➔ 파이 팝아웃 확대](#26-완성형-차트-계층-뷰-대분류-레이더--소분류-파이--파이-팝아웃-확대)
...
* **Progressive View Separation (대분류 따로 / 소분류 따로 계층형 뷰)**:
  1. **대분류 레이더 뷰**: 진입 시 대분류 레이더 다각형만 우선 노출하여 8대 카테고리의 밸런스를 명료하게 조망.
  2. **소분류 파이 뷰**: 레이더 차트를 탭하면 구성된 소분류 파이 차트로 전환되어 세부 향미들을 확인.
...
### 2.6 완성형 차트 계층 뷰: 대분류 레이더 ➔ 소분류 파이 ➔ 파이 팝아웃 확대

```
[ Step 1: 기본 진입 ]
  • 대분류 레이더 다각형만 깔끔하게 표시
  • 상단: "👆 대분류 레이더 차트를 탭하면 소분류 파이 차트로 전환됩니다"
        │
        ▼ (레이더 차트 내부 탭)
[ Step 2: 소분류 파이 차트 전개 ]
  • 각 대분류 섹터별 소분류 파이 조각(Pie Wedges)들이 렌더링
...
graph TB
    subgraph Client ["Flutter Client (Offline-First Interactive Architecture)"]
        ProgressiveCardEngine["대분류 레이더 ➔ 소분류 파이 ➔ 팝아웃 계층 뷰 엔진"]
...
| :--- | :--- | :--- | :---: | :--- |
| **FR-01** | **결과 렌더링** | **대분류/소분류 분리 뷰**| **P1** | 레이더만 우선 노출 ➔ 탭 시 파이 전환 ➔ 파이 탭 시 1.45x 확대 ➔ 바깥쪽 탭 시 복귀 |
...
| :--- | :--- | :--- | :---: |
| **1단계** | **설계 & 프로토타입** | • PRD 및 대분류 레이더 / 소분류 파이 분리 계층 뷰 확립<br>• 완성형 테이스팅 카드 HTML 프로토타입 | 🟢 **완료** |
...
* **완성형 차트 대분류/소분류 분리 뷰**:
  1. 하단 `[🏆 완성된 차트 보기]` 진입 시 **대분류 레이더 차트만 우선 표시**
