...
> *"면적 터치 위아래 드래그 조작, 점수 영역 탭 시 소분류 확대 진입, 4점 이하 적응형 확대 시 배경 눈금 동적 재스케일링, 하단 미니 다이얼 림, 그리고 완성된 차트에서 0점 미선택 대분류를 완전 제외한 맞춤형 대분류 레이더 ➔ 탭 시 소분류 파이 전환 ➔ 개별 파이 탭 시 대폭 확대(Mega Pop-out) 인터랙션을 제공합니다."*
...
2. [핵심 아키텍처 규칙 5대 원칙](#2-핵심-아키텍처-규칙-5대-원칙)
   ...
   - 2.6 [완성형 차트: 0점 제외 맞춤형 N각형 레이더 ➔ 소분류 파이 ➔ 파이 팝아웃 확대](#26-완성형-차트-0점-제외-맞춤형-n각형-레이더--소분류-파이--파이-팝아웃-확대)
...
* **Zero Filtered Dynamic Scored Card (0점 미선택 항목 완전 제외)**: 완성된 차트에서는 **0점(미선택) 항목은 방사선 축선, 외곽 라벨, 다각형 꼭짓점에서 완전히 제외**하고 실제 평가된 향미들로만 맞춤형 N각형 다각형과 파이 차트를 구성.
* **Progressive View Separation (대분류 따로 / 소분류 따로 계층형 뷰)**:
  1. **대분류 레이더 뷰**: 진입 시 평가된 대분류 레이더 다각형만 우선 노출하여 전체 밸런스를 깔끔하게 조망.
  2. **소분류 파이 뷰**: 레이더 차트를 탭하면 각 대분류 섹터별 소분류 파이 차트로 전환되어 세부 향미들을 확인.
  3. **개별 파이 팝아웃 확대**: 특정 파이를 탭하면 해당 파이가 1.45배 확 커져서(Mega Pop-out) 눈에 잘 보이게 하이라이트.
  4. **레이더 복귀**: 파이 바깥쪽을 누르면 다시 대분류 레이더 차트만 표시.
...
### 2.6 완성형 차트: 0점 제외 맞춤형 N각형 레이더 ➔ 소분류 파이 ➔ 파이 팝아웃 확대

```
[ Step 1: 기본 진입 (대분류 레이더 뷰) ]
  • 0점인 대분류는 완전히 제외된 맞춤형 N각형 레이더 다각형만 깔끔하게 표시!
  • 상단: "👆 대분류 레이더 차트를 탭하면 소분류 파이 차트로 전환됩니다"
        │
        ▼ (레이더 차트 내부 탭)
[ Step 2: 소분류 파이 차트 전환 (소분류 뷰) ]
  • 평가된 대분류 섹터별 소분류 파이 조각(Pie Wedges)들이 렌더링
...
graph TB
    subgraph Client ["Flutter Client (Offline-First Interactive Architecture)"]
        ProgressiveCardEngine["0점 제외 대분류 레이더 ➔ 소분류 파이 ➔ 팝아웃 계층 뷰 엔진"]
...
| :--- | :--- | :--- | :---: | :--- |
| **FR-01** | **결과 렌더링** | **0점 제외 대분류/소분류 뷰**| **P1** | 0점 대분류 완전 제외 + 레이더만 우선 노출 ➔ 탭 시 파이 전환 ➔ 파이 탭 시 1.45x 확대 ➔ 바깥쪽 탭 시 복귀 |
| **FR-02** | **인차트 조작** | **소분류 평가 시스템 유지**| **P1** | 휠 조작 시 면적 드래그, 0~10점 매핑, 탭 줌인, 바텀시트 태그 추가 완벽 유지 |
| **FR-03** | **인차트 조작** | **배경 눈금 동적 재스케일링**| **P1** | 4점 이하 확대 시 배경 눈금 숫자가 대분류 점수 기준(예: 1점, 2점, 3점 상한)으로 전환 |
| **FR-04** | **휠 네비게이션**| **하단 미니 반원 다이얼**| **P1** | 소분류 줌인 상태에서 하단 중앙 미니 반원을 돌려 대분류 손쉬운 전환 |
...
| :--- | :--- | :--- | :---: |
| **1단계** | **설계 & 프로토타입** | • PRD 및 0점 제외 대분류 레이더 / 소분류 파이 계층 뷰 확립<br>• 완성형 테이스팅 카드 HTML 프로토타입 | 🟢 **완료** |
...
* **0점 미선택 대분류 완전 제외**: 점수를 기입한 항목들로만 맞춤형 N각형 레이더 다각형 구성
* **완성형 차트 대분류/소분류 분리 뷰**:
  1. 하단 `[🏆 완성된 차트 보기]` 진입 시 **0점이 제외된 대분류 레이더 차트만 우선 표시**
  2. 레이더 차트를 탭하면 **소분류 파이 차트로 전환**
