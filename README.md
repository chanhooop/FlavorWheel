...
> *"면적 터치 위아래 드래그 조작, 점수 영역 탭 시 소분류 확대 진입, 대분류 뷰 내 소분류 쐐기 실시간 미리보기, 하단 미니 다이얼 림, 그리고 옥타곤 배경 360도 통합 레이더 차트 완성을 제공합니다."*
...
2. [핵심 아키텍처 규칙 5대 원칙](#2-핵심-아키텍처-규칙-5대-원칙)
   ...
   - 2.3 [대분류 점수 뷰 내 기입된 소분류 쐐기 영역 실시간 미리보기](#23-대분류-점수-뷰-내-기입된-소분류-쐐기-영역-실시간-미리보기)
   - 2.4 [소분류 줌인 상태 하단 중앙 미니 반원 다이얼 림](#24-소분류-줌인-상태-하단-중앙-미니-반원-다이얼-림)
...
### 2.3 대분류 점수 뷰 내 기입된 소분류 쐐기 영역 실시간 미리보기

* 1단계 휠 전체 뷰(`currentFlowPhase = 'WHEEL_NAV'`)에서도,
* 해당 대분류에 소분류들이 기입되어 있다면(`t1.subnodes.length > 0`),
* **대분류 점수 영역($0.0 \sim \text{intensity}$) 내부에 각 소분류 쐐기들이 점수 높이만큼 분할 채워진 모습**이 실시간으로 렌더링되어 전체 테이스팅 진행 상황을 한눈에 파악할 수 있습니다.

---

### 2.4 소분류 줌인 상태 하단 중앙 미니 반원 다이얼 림
...
| :--- | :--- | :--- | :---: | :--- |
| **FR-01** | **인차트 조작** | **대분류 내 소분류 미리보기**| **P1** | 대분류 점수 뷰에서도 기입된 소분류들이 쐐기 영역으로 나뉘어 실시간 표시 |
| **FR-02** | **인차트 조작** | **면적 드래그 & 탭 줌인**| **P1** | 면적 드래그로 점수 조절, 점수 영역 탭 시 소분류 확대로 진입 |
...
| :--- | :--- | :--- | :---: |
| **1단계** | **설계 & 프로토타입** | • PRD 및 대분류 내 소분류 쐐기 미리보기 규칙 확립<br>• 옥타곤 롱프레스 통합 HTML 프로토타입 | 🟢 **완료** |
